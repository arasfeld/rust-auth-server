...
use crate::repositories::{user_repository, user_secrets_repository};
...
pub async fn login(
    ...
    if username.contains("@") {
        // It's an email
        let user = user_repository::get_by_email(db, username)
            .await
            .unwrap();
        maybe_user = Some(user)
    } else {
        // It's a username
        let user = user_repository::get_by_username(db, username)
            .await
            .unwrap();
        maybe_user = Some(user)
    }
    ...
    if let Some(user) = maybe_user {
        // Load their secrets
        let user_secrets = user_secrets_repository::get_by_user_id(db, user.id).await.unwrap();
        ...
        if verify_password(&password_hash, password) {
            ...
            user_secrets_repository::reset_login_attempts(db, user.id).await.unwrap();
            return Ok(user);
        } else {
            user_secrets_repository::add_failed_login_attempt(db, user.id).await.unwrap();
        }
    }

    Err(AppError::InvalidUsernamePassword)
}
